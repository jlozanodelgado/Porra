...
export default function AdminResultForm({ matches }: { matches: Match[] }) {
    ...
    const [editFinishedId, setEditFinishedId] = useState<number | null>(null);
    ...
    const MatchResultFields = ({ m }: { m: Match }) => (
        <>
            ...
            <div className="flex gap-4 items-center mb-3">
                <div className="flex flex-col items-center">
                    ...
                    <input
                        ...
                        max={20}
                        required
                        defaultValue={m.home_goals_real ?? ''}
                        placeholder="0"
                ...
                <div className="flex flex-col items-center">
                    ...
                    <input
                        ...
                        max={20}
                        required
                        defaultValue={m.away_goals_real ?? ''}
                        placeholder="0"
                        className="w-16 bg-[var(--color-surface)] border border-white/20 p-2 rounded text-center text-white text-lg font-heading focus:border-[var(--color-neon-cyan)] focus:outline-none transition-colors"
                    />
                </div>
            ...
            <div className="flex gap-2">
                <button
                    type="submit"
                    disabled={isPending}
                    className="flex-1 bg-[var(--color-neon-cyan)] text-black px-4 py-2 font-bold rounded hover:brightness-110 shadow-lg transition-all disabled:opacity-50 text-sm"
                >
                    {isPending ? 'Calculando puntos...' : 'Guardar y Recalcular'}
                </button>
                {m.status === 'finished' && (
                    <button
                        type="button"
                        onClick={() => setEditFinishedId(null)}
                        className="text-gray-400 px-3 py-2 text-xs hover:text-white transition-colors"
                    >
                        Cancelar
                    </button>
                )}
                ...
                <>
                    ...
                    <div className="flex flex-col gap-2">
                        {finishedMatches.map((m) => (
                            <div key={m.id} className="bg-black/20 rounded-lg p-3 border border-white/5">
                                {editFinishedId === m.id ? (
                                    <form onSubmit={(e) => handleSubmit(e, m.id)}>
                                        <MatchResultFields m={m} />
                                    </form>
                                ) : (
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <button
                                                onClick={() => setEditFinishedId(m.id)}
                                                className="text-[var(--color-neon-cyan)] text-xs hover:underline"
                                            >
                                                Editar
                                            </button>
                                        </div>
