...
export default async function RulesPage() {
    ...
    if (user) {
        const { data } = await supabase
            .from('profiles')
            // ✅ Agregamos porra_id a la lista de columnas seleccionadas
            .select('display_name, nickname, is_admin, avatar_url, porra_id')
            .eq('id', user.id)
            .single();
        profile = data;
    }
    ...
    // Retorno de seguridad si no hay usuario autenticado
    if (!user) return null;
